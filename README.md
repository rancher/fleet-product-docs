...
```console
npx antora --fetch fleet-product-playbook.yml
```
...
```console
npx antora --fetch fleet-community-playbook.yml
```
...
As an example, run the command below to use the product `fleet-product-playbook.yml` file.

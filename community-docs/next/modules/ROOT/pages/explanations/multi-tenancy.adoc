...
[#_the_pre_existing_serviceaccount_requirement]
...
[#_the_default_fleet_default_serviceaccount]
...
NOTE: The behavior described in this section, the `allowNamespaceCreation` field, and applying `namespaceLabels` and `namespaceAnnotations` as the resolved `ServiceAccount` are available since {product_name} v0.16.1. In v0.16.0, Helm creates missing target namespaces regardless of `requireServiceAccount`, and the agent applies `namespaceLabels` and `namespaceAnnotations` with its own credentials.
...
[#_what_policy_deliberately_does_not_cover]
...
Splitting the responsibility this way means downstream RBAC remains the single source of truth for downstream authority. {product_name}'s job is to make sure the right `ServiceAccount` is used; the permissions of that `ServiceAccount` are the operator's job to set.

[#_why_requireserviceaccount_is_its_own_field]
...
[#_where_gitreporestriction_fits]

...
This does not apply when a `Policy` in the bundle's namespace sets `requireServiceAccount: true` without `allowNamespaceCreation: true`. The namespace must then exist before the deployment, see xref:explanations/multi-tenancy.adoc#_namespace_creation[Namespace Creation].

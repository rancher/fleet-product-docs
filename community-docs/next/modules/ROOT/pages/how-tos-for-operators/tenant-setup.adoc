...
Then the `GlobalRole`. The rules are split into two blocks: one for resources the tenant may write, and an optional block granting read-only access to `Policy`. Omit the `policies` block if you prefer tenants not to see `Policy` objects directly.

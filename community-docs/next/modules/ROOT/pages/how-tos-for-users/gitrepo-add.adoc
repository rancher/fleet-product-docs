...
It takes precedence over all other namespace definitions:

`gitRepo.targetNamespace > fleet.yaml namespace > namespace in workload's manifest > fleet.yaml defaultNamespace`

Your operator may restrict which `serviceAccount`, `clientSecretName`, and `repo` values your `GitRepo` may use via a `Policy` in your namespace. If a constraint is violated, the `GitRepo`'s status condition reports which field was rejected. Target-namespace restrictions are enforced by the downstream cluster's RBAC on the `ServiceAccount` you reference, not by {product_name} itself; if the `ServiceAccount` does not have permission to write to the target namespace, the deployment will fail on the downstream cluster. See xref:explanations/multi-tenancy.adoc[Multi-Tenancy] for the model.
...
[NOTE]
====
The private key format has to be in `EC PRIVATE KEY`, `RSA PRIVATE KEY` or `PRIVATE KEY` and should not contain a passphrase.
====
...
When that value is set to `false`, {product_name} will enforce strict host key checks, meaning that it will fail to establish any
SSH connections to hosts for which no matching `known_hosts` entry can be found.

This is the default behavior from {product_name} v0.13 onwards.
...
Note that this is compatible with {product_name} looking for a `gitcredential` secret if no secret is referenced in the
`GitRepo`.

If no such secret exists, or no `known_hosts` entries are available in that secret, then {product_name} uses its own
`known-hosts` config map, newly created at installation time with static entries for the most widely used git providers.

More details xref:how-tos-for-operators/installation.adoc#_additional_ssh_known_hosts[here] on where the default entries
come from and how to populate additional ones at installation time.

The absence of the config map, should no secret be available, is considered a symptom of an incomplete {product_name} deployment,
and reported as such.

{product_name} only uses a single_source of `known_hosts` entries at a time. This means that, even if a secret contains invalid
(or insufficient) entries, then {product_name} will not look for valid entries in the config map. This applies to a secret

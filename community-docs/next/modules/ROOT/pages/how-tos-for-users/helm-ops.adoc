...
[#_restrictions_your_operator_may_apply]
== Restrictions Your Operator May Apply

Your operator may restrict which `serviceAccount`, `helmSecretName`, `helm.repo` URL, and `helm.chart` values your `HelmOp` may use via a `Policy` in your namespace. If a constraint is violated, the `HelmOp`'s status condition reports which field was rejected. See xref:explanations/multi-tenancy.adoc[Multi-Tenancy] for the model.
...
[#_propagating_image_pull_secrets_to_downstream_clusters]
...
For more details on `downstreamResources`, including lifecycle and monitoring behavior, refer to

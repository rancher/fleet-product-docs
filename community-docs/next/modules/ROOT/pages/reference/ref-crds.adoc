...
[WARNING]
====
Deprecated. Use the xref:reference/ref-policy.adoc[Policy] custom resource instead. `GitRepoRestriction` will be removed in a future release. It has known security limitations: repo patterns are matched unanchored (a pattern like `github.com/myorg` also matches `evil.com/?ref=github.com/myorg`), and it does not cover `HelmOp` or `Bundle` resources. Migration is strongly recommended. See xref:explanations/multi-tenancy.adoc[Multi-Tenancy].
====
...
[WARNING]
====
Deprecated. Use the xref:reference/ref-policy.adoc[Policy] custom resource instead. `GitRepoRestriction` will be removed in a future release. See xref:explanations/multi-tenancy.adoc[Multi-Tenancy].
====

...
[WARNING]
====
Deprecated for multi-tenancy. Use the xref:reference/ref-policy.adoc[Policy] custom resource instead. GitRepoRestriction remains supported but is not a security boundary on its own. See xref:explanations/multi-tenancy.adoc[Multi-Tenancy].
====

...
spec:
  ...
  # secret can be set at the namespace level using the Policy type's
  # gitRepo.defaultClientSecretName field. Secrets must be of the type
  # "kubernetes.io/ssh-auth" or "kubernetes.io/basic-auth". The secret is
  # assumed to be in the same namespace as the GitRepo.

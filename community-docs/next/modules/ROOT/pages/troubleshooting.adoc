...
By default, {product_name} retries the operation a limited number of times. If conflicts occur frequently under heavy load, increase this value to allow more retry attempts.

You can configure this variable in the {product_name} controller deployment.

For example:


For more information about configuring {product_name} environment variables, see xref:reference/ref-configuration.adoc[{product_name} configuration].

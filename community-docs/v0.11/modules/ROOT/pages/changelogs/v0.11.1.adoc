...
{product_name} v0.11.1 improves several log and status messages. It reduces the amount of reconciles done by the controllers for resource changes.

This release adds k8s events for the GitRepo resource. {product_name} generates a k8s event that users can subscribe to, for the following events:
...
_Information retrieved from https://github.com/rancher/fleet/releases/tag/v0.11.1[{product_name} v0.11.1]_

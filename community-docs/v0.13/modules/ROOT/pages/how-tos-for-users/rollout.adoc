...
{product_name} evaluates the ++Ready++ status of each ++BundleDeployment++ to determine when to proceed to the next partition. For more information, refer to the xref:reference/ref-status-fields.adoc[Status fields].
...
{product_name} proceeds only if the number of ++NotReady++ partitions is less than ++maxUnavailablePartitions++.

[NOTE]
====
{product_name} rolls out deployments in batches of up to 50 clusters per partition. After each batch, {product_name} checks the ++maxUnavailable++ threshold. For example:

* For a partition with 25 clusters and ++maxUnavailable: 5++, {product_name} deploys to all 25 and then checks.
* For a partition with 100 clusters, {product_name} deploys to 50, checks, and then proceeds with the next 50.
====

image::flow-rollout-fleet.png[A visual asset displaying flow of rollout in {product_name}.]

{product_name} rollout strategy options:
...
[#_automatic_partitioning]
=== Automatic Partitioning

{product_name} automatically creates partitions using ++autoPartitionSize++. For 200 clusters with ++autoPartitionSize: 25%++, it creates 4 partitions of 50 clusters each.
...
{product_name} performs rollout as follows:
...
[Note]
====
When using manual partitions, Fleet only includes targets that match a configured partition selector in the rollout. Targets that do not match any partition selector are discarded, and no rollout is created for them.
====
...
If multiple partitions are defined, {product_name} uses ++maxUnavailablePartitions++ to control rollout.
...
{product_name} uses readiness checks to control rollout pace.
...
If clusters don’t divide evenly, {product_name} rounds down partition sizes.
...
* {product_name} deploys to 50, then evaluates readiness.
* Waits if 10 or more clusters are ++NotReady++.

[#_scenario_200_clusters_auto]
...
* {product_name} waits if 2 or more partitions are ++NotReady++.

[#_scenario_200_clusters_manual]
...
* {product_name} rolls out to ++demoRollout++.

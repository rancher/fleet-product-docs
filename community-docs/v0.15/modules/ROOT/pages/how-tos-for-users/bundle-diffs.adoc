...
[NOTE]
====
You may not need to create diff rules for replica count changes managed by an HPA. Since,  {product_name} automatically ignores changes to the replicas field for Deployment and StatefulSet resources when the values remain within the HPA-managed range.
====

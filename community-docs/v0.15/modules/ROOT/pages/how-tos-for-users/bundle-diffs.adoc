...
[NOTE]
====
Use ++diff.comparePatches++ in your ++fleet.yaml++ file to filter expected runtime changes. This helps ensure that a ++Modified bundle++ represents actual configuration drift that requires investigation.
====

[#_when_to_add_diffs]
== When to add diffs

Add a compare patch when Kubernetes components modify resources automatically in a predictable and harmless way after deployment. Some common scenarios include:

* Admission webhooks injecting certificates or configuration
** For example, a cluster can automatically inject a ++caBundle++ into a ++ValidatingWebhookConfiguration++.
* Controllers adding runtime defaults
** For example, a policy engine can inject default CPU limits or tolerations into a Deployment.
* Temporary lifecycle jobs that are automatically removed after completion
** For example, some Helm charts create temporary initialization jobs, that automatically delete themselves after completion.

[#_when_not_to_add_diffs]
== When not to add diffs

Do not use comparePatches to hide unexpected configuration drift or manual changes.

If an administrator manually edits or patches a {product_name}-managed resource, {product_name} marks the bundle as ++Modified++.

Do not ignore these changes with a diff rule. Instead either revert the manual change to match the Git repository or update the Git repository with the new desired state.

[NOTE]
====
You may not need to create diff rules for replica count changes managed by an HPA. Since v0.15.0, {product_name} automatically ignores changes to the replicas field for Deployment and StatefulSet resources when the values remain within the HPA-managed range.
====

[#_configure_compare_patches_example]
== Configure compare patches example

The following ++fleet.yaml++ example handles two common runtime modification scenarios:

* a controller injecting a certificate into a webhook
* a temporary initialization job
...
diff:
  comparePatches:
    ...
    # Ignore runtime-injected webhook fields
    ...
    # Ignore temporary jobs in the fleet-demo namespace
    - apiVersion: batch/v1
      kind: Job
...
In this example:
* {product_name} ignores runtime-generated webhook fields
* {product_name} ignores temporary jobs created and deleted in the `fleet-demo` namespace by the application lifecycle

[NOTE]
====
Instead of manually creating JSON Patch paths, use the ++fleet bundlediff++ CLI command. After you commit and push the updated ++fleet.yaml++, {product_name} reconciles the change and transitions the bundle from Modified to Ready.

====

[#_generating_comparepatches_with_fleet_bundlediff]
...
[#_generating_a_fleet_yaml_comparepatches_snippet]
...
[#_ignoring_object_modifications]
...
[#_1_validatingwebhookconfiguration]
...
[#_2_deployment_gatekeeper_controller_manager]
...
[#_3_deployment_gatekeeper_audit]

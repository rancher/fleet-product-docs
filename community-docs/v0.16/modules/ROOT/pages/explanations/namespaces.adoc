...
Since {product_name} v0.16.1, this does not apply when a `Policy` in the bundle's namespace sets `requireServiceAccount: true` without `allowNamespaceCreation: true`. The namespace must then exist before the deployment, see xref:explanations/multi-tenancy.adoc#_namespace_creation[Namespace Creation].
...
[IMPORTANT]
====
The `GitRepoRestriction` resource is deprecated and would be removed in a future release. For comprehensive multi-tenant environment governance, it is highly recommended to use the new `Policy` resource instead. For more information, refer to xref:_restricting_what_tenants_may_do_in_a_namespace[restricting tenant deployments for multi tenancy].
====
...
[#_restricting_what_tenants_may_do_in_a_namespace]

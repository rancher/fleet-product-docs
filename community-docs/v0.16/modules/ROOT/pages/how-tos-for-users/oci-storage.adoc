...
data:
  ...
  cacerts: <base64-encoded-pem-ca-bundle>
----
...
| `cacerts`         | PEM-encoded CA bundle for registries using custom or private Certificate Authorities. | Base64-encoded PEM data | When set, {product_name} merges these certificates with the system root CA pool for TLS validation against the OCI registry.
|===

[#_productname_example]

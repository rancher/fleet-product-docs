...
* <<GitRepoPolicySpec,GitRepoPolicySpec>>
* <<HelmOpPolicySpec,HelmOpPolicySpec>>
...
[WARNING]
====
Deprecation planned. Use the xref:reference/ref-policy.adoc[Policy] custom resource instead. `GitRepoRestriction` will be deprecated in v0.17 and may be removed as soon as v0.18. It has known security limitations: repo patterns are matched unanchored (a pattern like `github.com/myorg` also matches `evil.com/?ref=github.com/myorg`), and it does not cover `HelmOp` or `Bundle` resources. Migration is strongly recommended. See xref:explanations/multi-tenancy.adoc[Multi-Tenancy].
====
...
[WARNING]
====
Deprecation planned. Use the xref:reference/ref-policy.adoc[Policy] custom resource instead. `GitRepoRestriction` will be deprecated in v0.17 and may be removed as soon as v0.18. See xref:explanations/multi-tenancy.adoc[Multi-Tenancy].
====
...
Policy restricts what GitRepo, HelmOp, and Bundle resources in the same namespace may do. Top-level fields are checked by the GitRepo, HelmOp, and Bundle reconcilers. Sub-object fields (`gitRepo`, `helmOp`) are read only by their respective reconciler, and any default values they set are applied before the top-level validators run. Multiple Policy objects in the same namespace are aggregated with OR/union semantics, sorted by name. See xref:reference/ref-policy.adoc[Policy Resource] for the field-by-field semantics and the aggregation rules.
...
| RequireServiceAccount, when true, rejects any GitRepo, HelmOp, or Bundle whose ServiceAccount is empty after any defaulting has been applied.
| bool
| false

| allowedServiceAccounts
| AllowedServiceAccounts lists service accounts that may be used. If non-empty, the ServiceAccount must appear in this list. When RequireServiceAccount is also true, an empty ServiceAccount is rejected regardless of this list.
| []string
| false

| allowNamespaceCreation
| AllowNamespaceCreation, when true, allows Fleet to create namespaces on downstream clusters during Helm installs. By default (false), when RequireServiceAccount is true, namespace creation is disabled so that tenant ServiceAccounts are not forced to hold the cluster-scoped namespaces.create verb. Operators who have secured namespace creation through other means (ValidatingAdmissionPolicy, Kyverno, etc.) can set this to true.
| bool
| false
...
GitRepoPolicySpec holds GitRepo-specific defaults and source restrictions, applied only by the GitRepo reconciler before the top-level validators run.

|===
| Field | Description | Scheme | Required

| defaultServiceAccount
| DefaultServiceAccount is applied to GitRepo objects whose ServiceAccount is empty, before the top-level RequireServiceAccount check runs.
| string
| false

| defaultClientSecretName
| DefaultClientSecretName is applied to GitRepo objects whose ClientSecretName is empty.
| string
| false
...
| allowedRepoPatterns
| AllowedRepoPatterns is a list of regex patterns restricting the Repo field of GitRepo objects.
| []string
| false
|===
...
HelmOpPolicySpec holds HelmOp-specific defaults and source restrictions, applied only by the HelmOp reconciler before the top-level validators run.

|===
| Field | Description | Scheme | Required

| defaultServiceAccount
| DefaultServiceAccount is applied to HelmOp objects whose ServiceAccount is empty, before the top-level RequireServiceAccount check runs.
| string
| false

| defaultHelmSecretName
| DefaultHelmSecretName is applied to HelmOp objects whose HelmSecretName is empty.
| string
| false

| allowedHelmSecretNames
| AllowedHelmSecretNames lists credential secret names that HelmOp objects may reference.
| []string
| false

| allowedHelmRepoPatterns
| AllowedHelmRepoPatterns is a list of regex patterns restricting the spec.helm.repo field of HelmOp objects.
| []string
| false

| allowedChartPatterns
| AllowedChartPatterns is a list of regex patterns restricting the spec.helm.chart field of HelmOp objects.
| []string
| false
|===

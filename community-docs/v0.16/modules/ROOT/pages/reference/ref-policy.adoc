...
# Since Fleet v0.16.1. When true, Helm may create the target namespace on
# downstream clusters.
# Only has an effect together with requireServiceAccount: true, which
# disables namespace creation by default so that tenant service accounts do
# not need the cluster-scoped namespaces create verb. Set it to true only if
# namespace creation is secured by other means.
...
* `allowNamespaceCreation` is true if any Policy in the namespace sets it. See xref:explanations/multi-tenancy.adoc#_namespace_creation[Namespace Creation].
...
| —
| —
| applies (disables Helm namespace creation when `requireServiceAccount` is true and this field is false). Since {product_name} v0.16.1.
...
A Policy violation surfaces as a status condition on the offending GitRepo, HelmOp, or Bundle. The Bundle reconciler does not apply defaults. Apart from validating, it only resolves `allowNamespaceCreation` and writes the result into the BundleDeployments it creates. The GitRepo reconciler writes resolved defaults back to the GitRepo resource. The HelmOp reconciler applies defaults in memory and carries them into the generated Bundle; the HelmOp resource itself is left unchanged.

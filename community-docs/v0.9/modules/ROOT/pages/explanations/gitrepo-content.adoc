...
Fleet looks for the following files to determine how resources will be deployed.
...
determines how resources are customized per target.
...
When using Kustomize or Helm the `kustomization.yaml` or the `helm.values` will control how resource are

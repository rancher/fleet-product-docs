...
See xref:./gitrepo-targets#_defining_targets[Mapping to Downstream Clusters].
...
The bundle can target multiple clusters. It uses the same xref:./gitrepo-targets#_target_matching[targeting as the GitRepo].
Additional xref:./gitrepo-targets#_supported_customizations[customization options] can be specified per target:

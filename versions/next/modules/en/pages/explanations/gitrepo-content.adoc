...
| *fleet.yaml* | Any subpath | If any fleet.yaml is found a new xref:concepts.adoc[bundle] will be defined. This allows mixing charts, kustomize, and raw YAML in the same repo.
...
See xref:gitrepo-add.adoc#using-private-helm-repositories[Using Private Helm Repositories].
...
image::/img/FleetValuesStages.svg[]

The targeting step can treat values as templates using cluster info. More info: xref:ref-fleet-yaml.adoc#templating[].
...
Refer to xref:/how-tos-for-users/experimental-downstream-resource.adoc[experimental downstream resources] for more information.
====
...
See xref:gitrepo-targets.adoc#customization-per-cluster[].
...
See xref:ref-status-fields.adoc[].

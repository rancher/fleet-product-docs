...
If you are using Fleet in a xref:./concepts.adoc[single cluster] style, the namespace will always be *fleet-local*. Check xref:./namespaces#_cluster_registration_namespace_fleet_local[here] for more on the `fleet-local` namespace.
...
When using Kubernetes https://kubernetes.io/docs/tasks/administer-cluster/encrypt-data/[encryption at rest] and storing credentials in Git, it is recommended to configure the upstream cluster to include several Fleet CRDs in encryption resource list:
...
See Fleet Troubleshooting section xref:./troubleshooting.adoc[here].

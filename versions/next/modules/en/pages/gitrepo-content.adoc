...
Internally each scanned path will become a xref:./concepts.adoc[bundle] that Fleet will manage,
...
Hardened clusters should add the Fleet CRDs to the xref:gitrepo-add.adoc#_storing_credentials_in_git[list of resources encrypted at rest], on the upstream cluster, when storing credentials in the bundles.
====

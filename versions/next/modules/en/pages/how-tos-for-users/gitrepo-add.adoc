...
[NOTE]
====
Fleet does not support SSH proxy server authentication when cloning <<adding-a-private-git-repository,adding-a-private-git-repository>>or <<using-private-helm-repositories,using Private Helm Repositories>>. Use HTTPS authentication with a username and password or a personal access token.
====

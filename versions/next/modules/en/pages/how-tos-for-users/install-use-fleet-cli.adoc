...
You can deploy workloads without using GitRepos by applying them locally with the CLI. For example, use the link:https://github.com/rancher/fleet-examples[{product_name} examples repository]:
...
For more information, refer to link:https://www.suse.com/c/rancher_blog/fleet-multi-cluster-deployment-with-the-help-of-external-secrets/[{product_name}: Multi-Cluster Deployment with the Help of External Secrets].

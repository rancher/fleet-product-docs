...
====
Rancher has separate helm charts for <<_suse_rancher_prime_continous_delivery,{product_name}>> and uses a different repository.
====
...
Rancher has separate helm charts for <<_continous_delivery,{product_name}>> and uses a different repository.
...
ifeval::["{build-type}" == "product"]
After the `ClusterRegistrationToken` is created, <<_suse_rancher_prime_continous_delivery,{product_name}>> creates a corresponding `Secret` with the same name.
endif::[]

ifeval::["{build-type}" == "community"]
After the `ClusterRegistrationToken` is created, <<_continous_delivery,{product_name}>> creates a corresponding `Secret` with the same name.
endif::[]
...
====
ifeval::["{build-type}" == "product"]
If you are using <<_suse_rancher_prime_continous_delivery,{product_name}>> standalone without Rancher, it must be installed as described in xref:./installation.adoc#configuration-for-multi-cluster[installation details].
endif::[]

ifeval::["{build-type}" == "community"]
If you are using <<_continous_delivery,{product_name}>> standalone without Rancher, it must be installed as described in xref:./installation.adoc#configuration-for-multi-cluster[installation details].
endif::[]

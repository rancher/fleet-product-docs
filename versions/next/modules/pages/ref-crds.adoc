...
When a GitRepo is scanned it will produce one or more bundles. Bundles are a collection of resources that get deployed to one or more cluster(s). Bundle is the fundamental deployment unit used in <<_suse_rancher_prime_continous_delivery,{product_name}>>. endif::[]

ifeval::["{build-type}" == "community"]
When a GitRepo is scanned it will produce one or more bundles. Bundles are a collection of resources that get deployed to one or more cluster(s). Bundle is the fundamental deployment unit used in <<_continous_delivery,{product_name}>>.
endif::[] 
...
ifeval::["{build-type}" == "product"]
BundleTarget declares clusters to deploy to <<_suse_rancher_prime_continous_delivery,{product_name}>> will merge the BundleDeploymentOptions from customizations into this struct.

endif::[]

ifeval::["{build-type}" == "community"]
BundleTarget declares clusters to deploy to <<_continous_delivery,{product_name}>> will merge the BundleDeploymentOptions from customizations into this struct.
endif::[] 
...
ifeval::["{build-type}" == "product"]
BundleTargetRestriction is used internally by <<_suse_rancher_prime_continous_delivery,{product_name}>> and should not be modified. It acts as an allow list, to prevent the creation of BundleDeployments from Targets created by TargetCustomizations in `fleet.yaml``.
endif::[]

ifeval::["{build-type}" == "community"]
BundleTargetRestriction is used internally by <<_continous_delivery,{product_name}>> and should not be modified. It acts as an allow list, to prevent the creation of BundleDeployments from Targets created by TargetCustomizations in `fleet.yaml``.
endif::[] 
...
ifeval::["{build-type}" == "product"]
BundleDeployment is used internally by <<_suse_rancher_prime_continous_delivery,{product_name}>> and should not be used directly. When a Bundle is deployed to a cluster an instance of a Bundle is called a BundleDeployment. 
endif::[]

ifeval::["{build-type}" == "community"]
BundleDeployment is used internally by <<_continous_delivery,{product_name}>> and should not be used directly. When a Bundle is deployed to a cluster an instance of a Bundle is called a BundleDeployment. 
...
ifeval::["{build-type}" == "product"]
Cluster corresponds to a Kubernetes cluster. <<_suse_rancher_prime_continous_delivery,{product_name}>> deploys bundles to targeted clusters. Clusters to which <<_suse_rancher_prime_continous_delivery,{product_name}>> deploys manifests are referred to as downstream clusters. In the single cluster use case, the Fleet Kubernetes cluster is both the manager and downstream cluster at the same time.

endif::[]

ifeval::["{build-type}" == "community"]
Cluster corresponds to a Kubernetes cluster. <<_continous_delivery,{product_name}>>  deploys bundles to targeted clusters. Clusters to which <<_continous_delivery,{product_name}>>  deploys manifests are referred to as downstream clusters. In the single cluster use case, the Fleet Kubernetes cluster is both the manager and downstream cluster at the same time.
...
ifeval::["{build-type}" == "product"]
ClusterRegistration is used internally by <<_suse_rancher_prime_continous_delivery,{product_name}>> and should not be used directly.
endif::[]

ifeval::["{build-type}" == "community"]
ClusterRegistration is used internally by <<_continous_delivery,{product_name}>> and should not be used directly.
endif::[] 
...
ifeval::["{build-type}" == "product"]
Content is used internally by <<_suse_rancher_prime_continous_delivery,{product_name}>> and should not be used directly. It contains the resources from a bundle for a specific target cluster.
endif::[]

ifeval::["{build-type}" == "community"]
Content is used internally by <<_continous_delivery,{product_name}>> and should not be used directly. It contains the resources from a bundle for a specific target cluster.
endif::[] 
...
ifeval::["{build-type}" == "product"]
GitRepo describes a git repository that is watched by <<_suse_rancher_prime_continous_delivery,{product_name}>>. The resource contains the necessary information to deploy the repo, or parts of it, to target clusters.
endif::[]

ifeval::["{build-type}" == "community"]
GitRepo describes a git repository that is watched by <<_continous_delivery,{product_name}>>. The resource contains the necessary information to deploy the repo, or parts of it, to target clusters.
endif::[] 

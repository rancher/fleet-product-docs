...
If you are using {product_name} in a xref:explanations/concepts.adoc[single cluster] style, the namespace will always be *fleet-local*. Check xref:explanations/namespaces.adoc#_fleet-local-local-workspace-cluster-registration-namespace[cluster registration namespace] for more on the `fleet-local` namespace.

...
* plus the usual unsharded Fleet controller pod. That latter pod will be the only one containing agent management and cleanup containers.
...
At this point the Fleet manager should be ready. You can now xref:./cluster-registration.adoc[register clusters] and xref:./gitrepo-add.adoc#_create_gitrepo_instance[git repos] with
the Fleet manager.

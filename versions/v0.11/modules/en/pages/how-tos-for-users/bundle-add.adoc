...
If you would like to deploy in downstream clusters, you need to define targets. Targets work similarly to targets in `GitRepo`. Refer to xref:how-tos-for-users/gitrepo-targets.adoc#_defining-targets[Mapping to Downstream Clusters].
...
The bundle can target multiple clusters. It uses the same xref:how-tos-for-users/gitrepo-targets.adoc#_target-matching[targeting as the GitRepo].
Additional xref:how-tos-for-users/gitrepo-targets.adoc#_supported-customizations[customization options] can be specified per target:
...
The blog post on link:https://www.suse.com/c/rancher_blog/fleet-multi-cluster-deployment-with-the-help-of-external-secrets/[{product_name}: Multi-Cluster Deployment with the Help of External Secrets] has more information.

...
[#_gitrepo-stuck-in-git-updating-state]
=== `GitRepo` stuck in `Git Updating` state

In some cases, updating {product_name} might cause existing GitRepos to remain stuck in the `Git Updating` state, with a force update not resolving the issue.
...
This issue is fixed in {product_name} v0.12. In earlier versions, you can work around it as follows:

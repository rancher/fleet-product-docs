...
The targeting step can treat the values as a template and fill in information from the `clusters.fleet.cattle.io` resource. More information can be found in xref:./ref-fleet-yaml.adoc#_templating[Helm values templating].
...
See xref:gitrepo-targets.adoc#_customization_per_cluster[Mapping to Downstream Clusters] for more information and a list of supported customizations.

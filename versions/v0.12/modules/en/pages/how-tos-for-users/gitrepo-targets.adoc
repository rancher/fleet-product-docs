...
[WARNING]
====
Overriding the version of a Helm chart by using target customizations causes bundles to include *all* versions of the chart (both default and customized) to support all clusters.

This increases bundle size. Because Fleet stores bundles in etcd, large bundles may exceed the configured maximum blob size.

For more information, see link:https://github.com/rancher/fleet/issues/1650[this issue].
====

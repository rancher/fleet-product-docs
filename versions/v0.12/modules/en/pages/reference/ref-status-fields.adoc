...
* *Pending* : Bundles are queued for processing by the {product_name} controller.  
  This may occur if rollout is paused (Refer to xref:how-tos-for-users/rollout.adoc[Rollout Strategy]).
* *OutOfSync* : Bundles are synced from the {product_name} controller, but updated BundleDeployments haven’t yet been created, so downstream agents can’t sync changes.  

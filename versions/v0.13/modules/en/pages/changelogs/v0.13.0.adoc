...
* Fleet supports a new, user-driven bundle scan method for greater flexibility. The previous scan method remains supported. See xref:gitrepo-content.adoc#_alternative_scan_explicitly_defined_by_the_user[documentation].  
  (Contributed by @0xavi0, PR https://github.com/rancher/fleet/pull/3480[#3480])
* When using SSH to connect to a Git repository, Fleet checks host keys by default and rejects connections to unknown hosts. See xref:gitrepo-add.adoc#_known_hosts[documentation].  
  (Contributed by @weyfonk, PR https://github.com/rancher/fleet/pull/3523[#3523])
* Replica counts are now configurable for both controllers and agents. See xref:installation.adoc#_controller_and_agent_replicas[documentation].  
  (Contributed by @p-se, PR https://github.com/rancher/fleet/pull/3457[#3457])
* Fleet can now use a separate webhook secret for each GitRepo. See xref:webhook.adoc#_option_2_define_a_secret_for_each_gitrepo[documentation].  
...
* Fleet adds new GitJob metrics. See xref:observability.adoc#_metrics[documentation].  
...
See xref:helm-ops.adoc[Fleet documentation] for more details.
...
See xref:oci-storage.adoc[Fleet documentation] for more details.

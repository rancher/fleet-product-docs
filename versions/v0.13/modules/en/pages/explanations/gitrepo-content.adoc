...
| `Chart.yaml` | Relative to `path` or custom path from `fleet.yaml` | Resources are deployed as a Helm chart. See `fleet.yaml` for options. |
| `kustomization.yaml` | Relative to `path` or custom path from `fleet.yaml` | Resources are deployed using Kustomize. See `fleet.yaml` for options. |
| `fleet.yaml` | Any subpath | Defines a new xref:concepts.adoc[bundle]. Enables mixing Helm, Kustomize, and raw YAML in the same repo. |
| `*.yaml` | Any subpath | If no `Chart.yaml` or `kustomization.yaml` is found, YAML files are treated as Kubernetes manifests. |
| `overlays/{name}` | Relative to `path` | Used for raw YAML deployments to apply environment-specific customizations. |
|===

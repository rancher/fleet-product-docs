...
See xref:/how-tos-for-users/gitrepo-add.adoc#using-private-helm-repositories[Using Private Helm Repositories].
...
The targeting step can treat values as templates using cluster info. More info: xref:/reference/ref-fleet-yaml.adoc#templating[templates in fleet.yaml].
...
----
See xref:/how-tos-for-users/gitrepo-targets.adoc#customization-per-cluster[customization per cluster].

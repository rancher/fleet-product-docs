...
See also xref:gitrepo-content.adoc#using-valuesfrom[valuesFrom].
...
[NOTE]
====
If resources referenced through `downstreamResources` should stay on downstream clusters even after they are no longer targeted, xref:ref-bundle.adoc[keepResources] should be set to `true` on the HelmOp.
====

...
See xref:how-tos-for-users/gitrepo-add.adoc#_using-private-helm-repositories[Using Private Helm Repositories].
...
The targeting step can treat values as templates using cluster info. More info: xref:/reference/ref-fleet-yaml.adoc#_values-templating[templates in fleet.yaml].
...
The targeting step can treat the values as a template and fill in information from the `clusters.fleet.cattle.io` resource. For more information, refer to xref:reference/ref-fleet-yaml.adoc#_values-templating[Helm values templating].
...
Hardened clusters should add the Fleet CRDs to the xref:how-tos-for-users/gitrepo-add.adoc#_storing-credentials-in-git[list of resources encrypted at rest], on the upstream cluster, when storing credentials in the bundles.
...
----
See xref:how-tos-for-users/gitrepo-targets.adoc#_customization-per-cluster[customization per cluster].

...
:page-aliases: /multi-user

{product_name} uses Kubernetes RBAC where possible.

One addition on top of RBAC is the xref:explanations/namespaces.adoc#_restricting-gitrepos[`GitRepoRestriction`] resource, which can be used to control GitRepo resources in a namespace.
...
In each of the user's namespaces, as an admin create a xref:explanations/namespaces.adoc#_cross-namespace-deployments[`BundleNamespaceMapping`].

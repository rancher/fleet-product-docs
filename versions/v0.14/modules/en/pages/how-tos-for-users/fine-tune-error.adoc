...
NOTE: 
Overriding conditions only affects the default error mappings (refer to xref:#_default-error-mappings[Default error mappings]). {product_name} may still mark a resource as an error because other checks, such as those from the `kstatus` library, continue to run after your customization.

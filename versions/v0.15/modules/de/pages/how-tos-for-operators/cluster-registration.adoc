...
[#_manager-initiierte-registrierung]
...
[IMPORTANT]
====
Es ist nicht notwendig, den Fleet Manager für xref:how-tos-for-operators/installation.adoc#_configuration-for-multi-cluster[Multi-Cluster] zu konfigurieren, da der Downstream-Agent, den wir über Helm installieren, direkt mit der Kubernetes-API des Upstream-Clusters verbindet.

Die Agent-initiierte Registrierung wird normalerweise nicht mit Rancher verwendet.
====


[#_cluster-registrierungstoken-und-client-id]
...
[#_agent-f-r-einen-neuen-cluster-installieren]
...
Zuerst folgen Sie den <<_create-cluster-registration-tokens,Cluster-Registrierungstoken-Anweisungen>>, um das `values.yaml` zu erhalten, das das Cluster-Registrierungstoken zur Authentifizierung gegen den {product_name}-Cluster enthält.
...
[#_agent-f-r-einen-vordefinierten-cluster-installieren]
...
[#_cluster-registrierungstoken-erstellen]
...
[#_erstellen-sie-ein-neues-token]
...
[#_token-wert-abrufen-agent-values-yaml]
...
[IMPORTANT]
====
Wenn Sie {product_name} eigenständig _ohne Rancher_ verwenden, muss es wie in xref:how-tos-for-operators/installation.adoc#_configuration-for-multi-cluster[Installationsdetails] beschrieben installiert werden.

Die vom Manager initiierte Registrierung wird verwendet, wenn Sie einen Cluster vom Rancher-Dashboard hinzufügen.
====


[#_kubeconfig-geheimnis-erstellen]

...
HINWEIS:  +
Das Überschreiben von Bedingungen betrifft nur die Standardfehlerzuordnungen (siehe xref:#_default_error_mappings[Standardfehlerzuordnungen]). {product_name} kann weiterhin eine Ressource als Fehler markieren, da andere Überprüfungen, wie die aus der `kstatus` Bibliothek, nach Ihrer Anpassung weiterhin ausgeführt werden.

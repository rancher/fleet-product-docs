...
Die GitRepo Ressource beschreibt Git-Repositories, wie auf sie zugegriffen wird und wo sich die Bundles befinden.

Der Inhalt der Ressource entspricht der xref:reference/ref-crds.adoc#_gitrepospec[GitRepoSpec].
Für weitere Informationen zur Nutzung der GitRepo Ressource, z.B. wie man private Repositories überwacht, siehe xref:how-tos-for-users/gitrepo-add.adoc[Erstellen einer GitRepo Ressource].
...
* xref:how-tos-for-users/gitrepo-add.adoc#_use_different_helm_credentials_for_each_path[Erstellen einer GitRepo Ressource].
* xref:how-tos-for-users/webhook.adoc[Webhooks anstelle von Polling verwenden].

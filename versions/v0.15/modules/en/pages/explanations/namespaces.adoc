...
If you are using {product_name} in a xref:explanations/concepts.adoc[single cluster] style, the namespace will always be *fleet-local*.  Refer to the xref:#_cluster-registration-namespace-fleet-local[`fleet-local` namespaces section] for more information.

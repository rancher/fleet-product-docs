...
If you are using {product_name} in a xref:explanations/concepts.adoc[single cluster] style, the namespace will always be *fleet-local*.  Refer to the xref:#_cluster_registration_namespace_fleet_local[`fleet-local` namespaces section] for more information.

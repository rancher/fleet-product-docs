...
xref:#_create-cluster-registration-tokens[cluster registration token] and optionally a client ID. The cluster
...
First, follow the xref:#_create-cluster-registration-tokens[cluster registration token] to obtain the `values.yaml` which contains

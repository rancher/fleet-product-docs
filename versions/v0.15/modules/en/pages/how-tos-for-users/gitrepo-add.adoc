...
[NOTE]
====
{product_name} does not support SSH proxy server authentication when cloning xref:#_adding-a-private-git-repository[adding a private gitrepository]or xref:#_using-private-helm-repositories[using Private Helm Repositories]
...
If you are using {product_name} in a xref:explanations/concepts.adoc[single cluster] style, the namespace will always be *fleet-local*. Check xref:explanations/namespaces.adoc#_cluster-registration-namespace-fleet-local[cluster registration] for more on the `fleet-local` namespace.
...
More details xref:how-tos-for-operators/installation.adoc#_additional-ssh-known-hosts[here] on where the default entries
...
Create a secret containing username and password. You can replace the password with a personal access token if necessary. Also see xref:troubleshooting.adoc#_http-secrets-in-github[HTTP secrets in Github].
...
Helm-style bundles can use xref:explanations/gitrepo-content.adoc#_using-valuesfrom[valuesFrom] to read values from a secret in the downstream cluster.

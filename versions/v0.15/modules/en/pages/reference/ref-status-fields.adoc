...
* `state` : Represents the GitRepo’s state (for example, `GitUpdating`) or the highest BundleState per the xref:_state_ranking[State Ranking]. If `Ready`, it’s set to an empty value.

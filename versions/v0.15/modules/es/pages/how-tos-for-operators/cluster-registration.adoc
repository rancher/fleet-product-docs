...
[#_registro-iniciado-por-el-gerente]
...
[IMPORTANT]
====
No es necesario configurar el Fleet Manager para xref:how-tos-for-operators/installation.adoc#_configuration-for-multi-cluster[multi cluster], ya que el agente descendente que instalamos a través de Helm se conectará directamente a la API de Kubernetes del clúster en sentido ascendente.

El registro iniciado por el agente normalmente no se utiliza con Rancher.
====


[#_token-de-registro-de-cl-ster-e-id-de-cliente]
...
[#_instalar-agente-para-un-nuevo-cl-ster]
...
Primero, sigue las <<_create-cluster-registration-tokens,instrucciones del token de registro de clúster>> para obtener el `values.yaml` que contiene el token de registro para autenticar contra el clúster {product_name}.
...
[#_instalar-agente-para-un-cl-ster-predefinido]
...
[#_crear-tokens-de-registro-de-cl-ster]
...
[#_obtenci-n-del-valor-del-token-valores-yaml-del-agente]
...
[IMPORTANT]
====
Si estás utilizando {product_name} de forma independiente _sin Rancher_, debe ser instalado como se describe en xref:how-tos-for-operators/installation.adoc#_configuration-for-multi-cluster[detalles de instalación].

El registro iniciado por el Manager se utiliza cuando añades un clúster desde el panel de control de Rancher.
====


[#_crear-secreto-de-kubeconfig]

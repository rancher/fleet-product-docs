...
El recurso GitRepo describe los repositorios git, cómo acceder a ellos y dónde se ubican los bundles.

El contenido del recurso corresponde a xref:reference/ref-crds.adoc#_gitrepospec[GitRepoSpec].
Para más información sobre cómo utilizar el recurso GitRepo, por ejemplo, cómo monitorizar repositorios privados, consulta xref:how-tos-for-users/gitrepo-add.adoc[Crear un Recurso GitRepo].
...
* xref:how-tos-for-users/gitrepo-add.adoc#_use_different_helm_credentials_for_each_path[Crear un Recurso GitRepo].

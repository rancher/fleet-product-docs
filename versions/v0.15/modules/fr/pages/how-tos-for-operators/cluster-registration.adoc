...
[#_enregistrement-initi-par-le-gestionnaire]
...
[IMPORTANT]
====
Il n'est pas nécessaire de configurer le Fleet Manager pour xref:how-tos-for-operators/installation.adoc#_configuration-for-multi-cluster[multi cluster], car l'agent en aval que nous installons via Helm se connectera directement à l'API Kubernetes du cluster en amont.

L'enregistrement initié par l'agent n'est normalement pas utilisé avec Rancher.
====


[#_jeton-d-enregistrement-de-cluster-et-id-client]
...
[#_installer-l-agent-pour-un-nouveau-cluster]
...
Tout d'abord, suivez les <<_create-cluster-registration-tokens,instructions du jeton d'enregistrement de cluster>> pour obtenir le `values.yaml` qui contient le jeton d'enregistrement pour s'authentifier auprès du cluster {product_name}.
...
[#_installer-l-agent-pour-un-cluster-pr-d-fini]
...
[#_cr-er-des-tokens-d-enregistrement-de-cluster]
...
[#_obtention-de-la-valeur-du-token-agent-values-yaml]
...
[IMPORTANT]
====
Si vous utilisez {product_name} en mode autonome _sans Rancher_, il doit être installé comme décrit dans xref:how-tos-for-operators/installation.adoc#_configuration-for-multi-cluster[détails d'installation].

L'enregistrement initié par le Manager est utilisé lorsque vous ajoutez un cluster depuis le tableau de bord Rancher.
====


[#_cr-er-un-secret-kubeconfig]
...
[#_cr-er-une-ressource-de-grappe]

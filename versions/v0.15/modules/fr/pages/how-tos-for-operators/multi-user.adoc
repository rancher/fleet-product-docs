...
Un ajout en plus de RBAC est la ressource xref:explanations/namespaces.adoc#_restricting-gitrepos[`GitRepoRestriction`], qui peut être utilisée pour gérer les ressources GitRepo dans un espace de noms.
...
[#_exemple-product-name-autonome]
...
[#_espaces-de-travail-isol-s-dans-rancher]
...
[#_autoriser-l-acc-s-aux-clusters]
...
Dans chacun des espaces de noms de l'utilisateur, en tant qu'administrateur, créez un xref:explanations/namespaces.adoc#_cross-namespace-deployments[`BundleNamespaceMapping`].
...
[#_restriction-de-l-acc-s-aux-clusters-en-aval]
...
[#_un-exemple-de-ressource-gitrepo]

...
[#_ignore-conditions-in-fleet-yaml]
== Ignorer les conditions dans `fleet.yaml`
...
[#_configure-error-mapping-with-environment-variables]
...
[#_disable-error-interpretation-example]
...
REMARQUE :  +
Le remplacement des conditions n'affecte que les mappages d'erreurs par défaut (voir xref:#_default-error-mappings[mappages d'erreurs par défaut]). {product_name} peut toujours marquer une ressource comme une erreur car d'autres vérifications, telles que celles de la bibliothèque `kstatus`, continuent de s'exécuter après votre personnalisation.

[#_enable-error-interpretation-example]

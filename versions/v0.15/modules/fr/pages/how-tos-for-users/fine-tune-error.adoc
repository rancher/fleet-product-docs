...
[#_ignore_conditions_in_fleet_yaml]
== Ignorer les conditions dans `fleet.yaml`
...
[#_configure_error_mapping_with_environment_variables]
...
[#_disable_error_interpretation_example]
...
REMARQUE :  +
Le remplacement des conditions n'affecte que les mappages d'erreurs par défaut (voir xref:#_default_error_mappings[mappages d'erreurs par défaut]). {product_name} peut toujours marquer une ressource comme une erreur car d'autres vérifications, telles que celles de la bibliothèque `kstatus`, continuent de s'exécuter après votre personnalisation.

[#_enable_error_interpretation_example]

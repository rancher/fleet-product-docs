...
La ressource GitRepo décrit les dépôts git, comment y accéder et où se trouvent les bundles.

Le contenu de la ressource correspond à la xref:reference/ref-crds.adoc#_gitrepospec[GitRepoSpec].
Pour plus d'informations sur la manière d'utiliser la ressource GitRepo, par exemple sur la façon de surveiller les dépôts privés, voir xref:how-tos-for-users/gitrepo-add.adoc[Créer une ressource GitRepo].
...
* xref:how-tos-for-users/gitrepo-add.adoc#_use_different_helm_credentials_for_each_path[Créer une ressource GitRepo].

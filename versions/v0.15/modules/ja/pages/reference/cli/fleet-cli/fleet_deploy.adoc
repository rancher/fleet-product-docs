...
Helmリリースを作成することによって、バンドルデプロイメント/コンテンツリソースをクラスターにデプロイする。これは、バンドルデプロイメント/コンテンツリソースをクラスターに直接デプロイすることはありません。
...
* xref:/reference/cli/fleet-cli/fleet.adoc[Fleet]     - {product_name} 操作を管理する

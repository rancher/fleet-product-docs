...
GitRepoリソースは、gitリポジトリの説明、アクセス方法、およびバンドルの場所を示しています。

リソースの内容は、xref:reference/ref-crds.adoc#_gitrepospec[GitRepoSpec]に対応しています。
GitRepoリソースの使い方、たとえばプライベートリポジトリの監視方法の詳細については、xref:how-tos-for-users/gitrepo-add.adoc[Create a GitRepo Resource]を参照してください。
...
* xref:how-tos-for-users/gitrepo-add.adoc#_use-different-helm-credentials-for-each-path[Create a GitRepo Resource]。
* xref:how-tos-for-users/webhook.adoc[ポーリングの代わりにWebhooksを使用する]。

...
xref:troubleshooting.adoc#_fetch-the-log-from-fleet-controller[`fleet-controller` ログ] と xref:troubleshooting.adoc#_fetch-the-log-from-the-fleet-agent[`fleet-agent` ログ] を確認してください。
...
`cattle-system`ネームスペースに`rancher-config`という設定マップをxref:reference/ref-configuration.adoc#_configure-product-install-options-in-rancher[Fleetインストールオプション]で作成できます。
...
このフィールドを無視するために、`fleet.yaml`でパッチを定義する必要があります。これはxref:troubleshooting.adoc#_gitrepo-or-bundle-stuck-in-modified-state[`GitRepo`または`Bundle`が修正状態で停止している]に従います。

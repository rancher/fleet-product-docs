...
[#_registro-iniciado-pelo-gerente]
...
[IMPORTANT]
====
Não é necessário configurar o Fleet Manager para xref:how-tos-for-operators/installation.adoc#_configuration-for-multi-cluster[multi cluster], pois o agente downstream que instalamos via Helm se conectará diretamente à API do Kubernetes do cluster upstream.

O registro iniciado pelo agente normalmente não é utilizado com o Rancher.
====


[#_token-de-registro-do-cluster-e-id-do-cliente]
...
[#_instalar-agente-para-um-novo-cluster]
...
Primeiro, siga as instruções do <<_create-cluster-registration-tokens,token de registro do cluster>> para obter o `values.yaml` que contém o token de registro para autenticar contra o cluster {product_name}.
...
[#_instalar-agente-para-um-cluster-predefinido]
...
[#_criar-tokens-de-registro-de-cluster]
...
[#_obtendo-o-valor-do-token-valores-yaml-do-agente]
...
[IMPORTANT]
====
Se você estiver usando {product_name} de forma independente _sem o Rancher_, ele deve ser instalado conforme descrito em xref:how-tos-for-operators/installation.adoc#_configuration-for-multi-cluster[detalhes de instalação].

O registro iniciado pelo gerenciador é utilizado quando você adiciona um cluster no painel do Rancher.
====


[#_criar-segredo-kubeconfig]

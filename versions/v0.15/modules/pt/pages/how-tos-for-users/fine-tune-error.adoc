...
NOTA:  +
Substituir condições afeta apenas os mapeamentos de erro padrão (consulte xref:#_default_error_mappings[Mapeamentos de erro padrão]). {product_name} ainda pode marcar um recurso como um erro porque outras verificações, como aquelas da biblioteca `kstatus`, continuam a ser executadas após sua personalização.

...
GitRepo 资源描述了git仓库、如何访问它们以及捆绑包的位置。

该资源的内容对应于xref:reference/ref-crds.adoc#_gitrepospec[GitRepoSpec]。
有关如何使用 GitRepo 资源的更多信息，例如如何监视私有仓库，请参见xref:how-tos-for-users/gitrepo-add.adoc[Create a GitRepo Resource]。
...
* xref:how-tos-for-users/webhook.adoc[使用 Webhooks 而不是轮询]。

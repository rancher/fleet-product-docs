...
检查 xref:troubleshooting.adoc#_fetch-the-log-from-fleet-controller[`fleet-controller` 日志] 和 xref:troubleshooting.adoc#_fetch-the-log-from-the-fleet-agent[`fleet-agent` 日志]。
...
您可以在 `cattle-system` 名称空间中创建一个配置映射 `rancher-config`，并使用 xref:reference/ref-configuration.adoc#_configure-product-install-options-in-rancher[Fleet 安装选项]。
...
您必须在 `fleet.yaml` 中定义一个补丁，以根据 xref:troubleshooting.adoc#_gitrepo-or-bundle-stuck-in-modified-state[`GitRepo` 或 `Bundle` 卡在修改状态] 忽略此字段。

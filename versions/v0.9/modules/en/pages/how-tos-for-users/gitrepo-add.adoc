...
If you are using {product_name} in a xref:explanations/concepts.adoc[single cluster] style, the namespace will always be *fleet-local*.  Refer to the xref:explanations/namespaces.adoc#_fleet_local_local_workspace_cluster_registration_namespace[`fleet-local` namespaces section] for more information.

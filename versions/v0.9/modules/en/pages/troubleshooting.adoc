...
Check the link:./troubleshooting.adoc#fetch-the-log-from-fleet-controller[`fleet-controller` logs] and the link:./troubleshooting.adoc#fetch-the-log-from-the-fleet-agent[`fleet-agent` logs].
...
If your GitJob returns the error below, the problem may be that Fleet cannot access the Helm repo you specified in your xref:./ref-fleet-yaml.adoc[`fleet.yaml`]:
